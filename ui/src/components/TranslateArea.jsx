    ...
    return (
        <section id="tranneutral-view" className="flex-1 flex flex-col h-full bg-transparent overflow-y-auto custom-scrollbar p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
                {/* SOURCE CARD */}
                <div className="bg-[#0a0a0a] border border-[#262626]/50 rounded-2xl p-5 flex flex-col space-y-4 shadow-xl">
                    ...
                    <div className="flex items-center justify-between border-t border-[#1f2937]/50 pt-3 text-xs text-neutral-500">
                ...
                {/* TARGET CARD */}
                <div className="bg-[#0a0a0a] border border-[#262626]/50 rounded-2xl p-5 flex flex-col space-y-4 shadow-xl relative">
                    <div className="flex items-center justify-between border-b border-[#1f2937]/50 pb-3">
                        <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-2">
                            {sourceLang !== 'Auto' && (
                                <button 
                                    onClick={handleSwap}
                                    className="p-1.5 rounded-lg bg-[#1f2937] hover:bg-[#2e3e56] text-neutral-400 hover:text-white transition-all"
                                    title="Tukar Bahasa"
                                >
                    ...
                    <div className="flex-1 min-h-[144px] text-sm text-neutral-200 leading-relaxed overflow-y-auto custom-scrollbar select-text whitespace-pre-wrap">
                        {isTranslating ? (
                            <div className="flex flex-col items-center justify-center h-full text-neutral-500 gap-2 font-medium">
                            ...
                            <span className="text-neutral-600 italic">Hasil terjemahan akan muncul di sini...</span>
                        )}
                    </div>

                    <div className="flex items-center justify-between border-t border-[#1f2937]/50 pt-3 text-xs">
                        <span className="text-neutral-500">{translatedText ? `${translatedText.length} karakter` : ""}</span>
                        {translatedText && (
                            <button 
                                onClick={() => copyToClipboard(translatedText, 'translation')}
                                className="text-white hover:text-neutral-300 font-semibold flex items-center gap-1.5 transition-colors"
                            >
            ...
            {(replies.length > 0 || isTranslating) && (
                <div className="bg-[#0a0a0a]/80 border border-[#262626]/40 rounded-2xl p-5 shadow-xl space-y-4">
                    ...
                    {isTranslating ? (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {[1, 2, 3].map((n) => (
                                <div key={n} className="bg-[#1f2937]/30 border border-neutral-800/80 rounded-xl p-4 h-24 animate-pulse flex flex-col justify-between">
                                    <div className="h-3.5 bg-neutral-700/50 rounded w-5/6"></div>
                                    <div className="h-3 bg-neutral-700/35 rounded w-1/2 mt-2"></div>
                                </div>
                            ))}
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            {replies.map((reply, idx) => (
                                <button
                                    ...
                                    className="bg-[#1f2937]/40 hover:bg-[#202c3f]/80 border border-neutral-800/80 hover:border-white/30 rounded-xl p-4 text-left transition-all relative group flex flex-col justify-between h-auto min-h-[96px] shadow-sm active:scale-[0.99]"
                                >
                                    <span className="text-xs text-neutral-300 group-hover:text-neutral-100 font-medium leading-relaxed select-text">{reply}</span>
                                    <div className="mt-4 flex items-center justify-between w-full">
                                        ...
                                        <svg className={`w-3.5 h-3.5 text-neutral-500 group-hover:text-white transition-colors ${copiedReplyIdx === idx ? 'text-white' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            {copiedReplyIdx === idx ? (
