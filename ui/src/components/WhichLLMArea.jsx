...
export default function WhichLLMArea() {
    ...
    return (
        <section id="whichllm-view" className="flex-1 overflow-y-auto bg-transparent p-8 custom-scrollbar">
            <div className="max-w-4xl mx-auto">
                <div className="mb-10">
                    ...
                    <p className="text-slate-400">Hardware Detection & Model Recommendations for your local machine.</p>
                </div>

                {isLoading ? (
                    <div className="text-center py-20 flex flex-col items-center justify-center">
                        <svg className="animate-spin h-8 w-8 text-cyan-400 mb-4" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                        <span className="text-cyan-400 font-bold uppercase tracking-wider text-xs">Detecting Hardware...</span>
                    </div>
                ) : hwInfo ? (
                    <div className="space-y-8">
                        {/* Hardware Specs Box */}
                        <div className="bg-gradient-to-br from-cyan-950/40 to-indigo-950/20 border border-cyan-500/30 p-8 rounded-3xl relative overflow-hidden group">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
                                <div className="bg-[#111827]/60 rounded-2xl p-5 border border-[#1e293b]">
                                    <div className="text-slate-500 text-[10px] font-bold uppercase tracking-wider mb-1">Total System RAM</div>
                                    <div className="text-3xl font-black text-white">{hwInfo.ram_gb} <span className="text-lg text-slate-500 font-medium">GB</span></div>
                                </div>
                                <div className="bg-[#111827]/60 rounded-2xl p-5 border border-[#1e293b]">
                                    <div className="text-slate-500 text-[10px] font-bold uppercase tracking-wider mb-1">Available Free RAM</div>
                                    <div className="text-3xl font-black text-cyan-400">{hwInfo.free_ram_gb} <span className="text-lg text-cyan-500/50 font-medium">GB</span></div>
                                </div>
                                <div className="bg-[#111827]/60 rounded-2xl p-5 border border-[#1e293b]">
                                    <div className="text-slate-500 text-[10px] font-bold uppercase tracking-wider mb-1">Detected GPU</div>
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                {getRecommendations(hwInfo.free_ram_gb, hwInfo.ram_gb).map((rec, i) => (
                                    <div key={i} className="bg-[#0f172a]/40 border border-[#1e293b] rounded-2xl p-5 hover:bg-[#1e293b]/40 transition-colors">
                                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-2">
                                            <div className="text-base font-bold text-slate-200">{rec.model}</div>
                                            <div className="bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 text-[10px] uppercase font-bold px-3 py-1 rounded-full w-fit">Requires {rec.memory}</div>
                                        </div>
                                        <p className="text-xs text-slate-400">{rec.note}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                        
                        <div className="bg-emerald-900/20 border border-emerald-500/30 p-5 rounded-xl text-xs text-emerald-200">
