    ...
    <section id="rag-view" className="flex-1 flex flex-col h-full bg-transparent p-6 space-y-6 overflow-y-auto custom-scrollbar">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Unggah dokumen yang akan digunakan ROBIT sebagai sumber pengetahuan.</p>
        </div>
        <div className="bg-cyan-500/10 border border-cyan-500/20 px-3 py-1.5 rounded-lg text-xs text-cyan-400 font-semibold flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-2xl p-8 transition-all flex flex-col items-center justify-center text-center cursor-pointer group min-h-[200px] ${
            dragActive ? 'border-cyan-500 bg-cyan-500/5 scale-[1.01]' : 'bg-[#0f172a]/60 border-[#22304d] hover:border-cyan-500/50 hover:bg-[#0f172a]/80'
          }`}
        >
          <input
            ref={fileInputRef}
          ...
          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-4 transition-all ${
            dragActive ? 'bg-cyan-500/20 text-cyan-400' : 'bg-[#1e293b]/60 text-slate-400 group-hover:bg-cyan-500/10 group-hover:text-cyan-400'
          }`}>
          ...
          <div className="mt-4 flex flex-wrap gap-1.5 justify-center">
            {['PDF', 'TXT', 'MD', 'JSON', 'CSV', 'PY', 'JS', 'TS'].map(ext => (
              <span key={ext} className="text-[10px] bg-[#1e293b]/60 border border-slate-700/50 text-slate-400 px-2 py-0.5 rounded font-mono">{ext}</span>
            ))}
          </div>
        </div>

        {/* Pipeline Ingestion Visualizer */}
        <div className="bg-[#0f172a]/40 border border-[#1e293b] rounded-2xl p-5 flex flex-col justify-between min-h-[200px]">
          <div>
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-1 flex items-center gap-2">
              <span className={`w-1.5 h-1.5 rounded-full ${uploadQueue.length > 0 ? 'bg-cyan-400 animate-ping' : 'bg-indigo-400/50'}`}></span>
          ...
          {uploadQueue.length > 0 ? (
            <div className="flex flex-col gap-2.5 flex-1 overflow-y-auto max-h-36 custom-scrollbar pr-1">
              {uploadQueue.map(entry => (
                <div key={entry.uid} className="bg-[#111827] border border-[#1e293b] rounded-xl px-4 py-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[11px] font-semibold text-slate-300 truncate max-w-[60%]">{entry.name}</span>
                    <span className={`text-[10px] font-bold ${
                      entry.status === 'done' ? 'text-emerald-400' :
                      entry.status === 'error' ? 'text-rose-400' : 'text-cyan-400'
                    }`}>{entry.message}</span>
                  </div>
                  {/* Progress bar */}
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-3 py-4">
              <div className="grid grid-cols-3 gap-2 w-full relative">
                <div className="absolute top-[18px] left-[18%] right-[18%] h-[2px] bg-slate-800" />
                {['Text Ingestion', 'Chunk Split', 'Vector Embed'].map((label, i) => (
                  <div key={label} className="flex flex-col items-center text-center z-10 gap-2">
                    <div className="w-9 h-9 rounded-full border bg-[#1e293b] border-slate-700 text-slate-600 flex items-center justify-center text-xs font-bold">{i + 1}</div>
      ...
      {/* Documents Table */}
      <div className="bg-[#0f172a]/40 border border-[#1e293b] rounded-2xl overflow-hidden flex-1 flex flex-col">
        <div className="px-5 py-4 border-b border-[#1e293b] flex items-center justify-between shrink-0">
          <h3 className="text-xs font-bold uppercase tracking-wider text-slate-300">Semua Dokumen Terdaftar</h3>
          <span className="text-[10px] text-slate-500">Sinkronisasi otomatis dengan TurboVec DB</span>
        </div>
        <div className="overflow-x-auto overflow-y-auto flex-1 custom-scrollbar">
          <table className="w-full text-left text-xs text-slate-300">
            <thead className="bg-[#0c1222]/80 uppercase text-[10px] tracking-wider text-slate-500 sticky top-0">
              <tr>
            ...
            <tbody className="divide-y divide-[#1e293b]/40">
              {kbDocs.length === 0 ? (
                ...
                kbDocs.map(doc => {
                  ...
                  return (
                    <tr key={doc.filename} className="hover:bg-[#1e293b]/20 transition-colors group">
                      <td className="px-6 py-4 font-semibold text-slate-100 max-w-xs truncate" title={doc.filename}>
                      ...
                      <td className="px-6 py-4">
                        <span className="bg-[#1e293b] px-2 py-1 rounded text-slate-300 text-[10px] font-bold font-mono">{ext}</span>
                      </td>
                      <td className="px-6 py-4 text-slate-500 text-[11px]">{doc.upload_time || '—'}</td>
                      <td className="px-6 py-4 text-center">
                        <button
                          onClick={() => onToggleKBDoc(doc.filename, !doc.active)}
                          className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors focus:outline-none ${doc.active ? 'bg-cyan-500' : 'bg-slate-600'}`}
                        >
                          <span
                        ...
                        <span className={`ml-2 text-[10px] font-bold ${doc.active ? 'text-cyan-400' : 'text-slate-500'}`}>
                      ...
                      <td className="px-6 py-4 text-right">
                        <button
                          ...
                          className="text-xs text-slate-500 hover:text-rose-400 font-semibold transition-colors group-hover:text-rose-400/60"
                        >
                          Hapus
                        </button>
                      </td>
